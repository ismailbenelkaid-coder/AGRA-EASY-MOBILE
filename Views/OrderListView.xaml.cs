...
public partial class OrderListView : ContentPage, INotifyPropertyChanged
{
    private const string DefaultSortLabel = "Par défaut";
    private const string ArticleSortLabel = "Article";
    private const string StatusSortLabel = "Status";
    ...
    private SorderLine? _currentDetailHeader;
    ...
    public SorderLine? CurrentDetailHeader
    {
    ...
    private void InitializeSortPicker()
    {
        ...
        PickerSortMode.Items.Add(StatusSortLabel);
    ...
    private async Task RefreshDataAsync()
    {
        ...
        try
        {
            var rawData = await EasySession.GetSordersLinesAsync(GlobalState.CurrentExpeditionFilter);
            if (rawData != null)
            {
                var orderedData = ApplySort(rawData);
                OrderCollection.ItemsSource = orderedData;
            }
    ...
    private SorderLine[] ApplySort(SorderLine[] data)
    {
        if (_selectedSortMode == DefaultSortLabel)
            return data.OrderByDescending(x => x.CreationDate).ThenBy(x => x.SorderCode ?? string.Empty).ThenBy(x => x.ProductCode ?? string.Empty).ToArray();

        IOrderedEnumerable<SorderLine> orderedData = _selectedSortMode switch
        {
            ArticleSortLabel => _sortAscending
                ? data.OrderBy(x => x.ProductCode ?? string.Empty).ThenBy(x => x.SorderCode ?? string.Empty)
                : data.OrderByDescending(x => x.ProductCode ?? string.Empty).ThenByDescending(x => x.SorderCode ?? string.Empty),

            StatusSortLabel => _sortAscending
                ? data.OrderBy(x => x.Status ?? string.Empty).ThenBy(x => x.ProductCode ?? string.Empty)
                : data.OrderByDescending(x => x.Status ?? string.Empty).ThenByDescending(x => x.ProductCode ?? string.Empty),
            ...
            ContainerSortLabel => _sortAscending
                ? data.OrderBy(x => x.Container ?? string.Empty).ThenBy(x => x.ProductCode ?? string.Empty)
                : data.OrderByDescending(x => x.Container ?? string.Empty).ThenByDescending(x => x.ProductCode ?? string.Empty),
    ...
    private async Task LoadDetailLinesAsync(SorderLine item)
    {
        SetLoadingState(true);
        try
        {
            var details = await EasySession.GetSorderLinesAsync(item.SorderCode, item.Warehouse, item.OriginalWarehouse);
            if (details != null)
            {
                var orderedDetails = ApplySort(details);
                _isSingleDetailView = true;
                CurrentDetailHeader = orderedDetails.FirstOrDefault() ?? item;
                OrderCollection.ItemsSource = orderedDetails;
                OnPropertyChanged(nameof(ShowDetailedView));
                OnPropertyChanged(nameof(ShowRepeatedHeader));
                OnPropertyChanged(nameof(ShowDetailsToggle));
                OnPropertyChanged(nameof(ShowBackButton));
    ...
    {
        var item = (SorderLine)((Button)sender).CommandParameter;
    ...
    private async void OnDownloadPdfClicked(object sender, EventArgs e)
    {
        ...
        try
        {
            byte[] pdf = await EasySession.GetSorderDocumentAsync(item.SorderCode, item.Warehouse, item.OriginalWarehouse);
            ...
            string safeSorderCode = string.Concat((item.SorderCode ?? "Commande").Select(c => Path.GetInvalidFileNameChars().Contains(c) ? '_' : c));
